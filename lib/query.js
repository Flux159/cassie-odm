...
//        console.log("THIS IS PARAMS AND QUERY STRING");
//        console.log(params);
//        console.log(queryString);

        if(params.length > 0) {
            connection.execute(queryString, params, {consistency: execOptions.consistency, prepare: true}, function (err, results, metadata) {
                if (execOptions.timing) {
                    var elapsed = process.hrtime(start);
                    var milli = elapsed[1] / 1000000; //millisecond time
            ...
            connection.execute(queryString, {consistency: execOptions.consistency, prepare: true}, function (err, results, metadata) {
                if (execOptions.timing) {
                    var elapsed = process.hrtime(start);
                    var milli = elapsed[1] / 1000000; //millisecond time
...
Query.batch = function (cassieQueries, connection, options, callback) {
    ...
    connection.batch(queries, options.consistency, {counter: options.counter}, function (err) {
        if (options.timing) {
            var elapsed = process.hrtime(start);
            var milli = elapsed[1] / 1000000; //millisecond time
