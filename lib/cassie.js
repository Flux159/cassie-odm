...
/* Creates a model w/ modelName: Cassie.model('User', UserSchema)
 * Returns a Model object based on the schema it created: Cassie.model('User')
 * Options: {connection: 'A connection returned from makeConnection, will be passed to model'}
 * Model internal Cassie.connection if not specified (throws error if no connection has been made)	
 */
exports.model = function(modelName, schema, options) {
	...
	Cassie.models[modelName] = Model.create(schema, (options.connection || Cassie.connection || null));
