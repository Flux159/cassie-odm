...
//connection.eachRow(query, params, consistency, rowCallback, endCallback) - stream each row
//connection.streamField(query, params, consistency, rowCallback, endCallback) - stream each field
...
//just need to expose the 4 C++ methods to Javascript (Cassie will hold the object in memory via the 'require' of the native module)
//Alternatively, might need JS to C++ types instead of JS to direct binary types. would have to check.
...
//    connection.execute(describeTablesQuery, describeTablesArguments, );
