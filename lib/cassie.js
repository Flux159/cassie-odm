...
exports.checkKeyspace = function(config, callback) {
	Sync.checkKeyspaceExists(config.hosts, config.keyspace, config, callback);
}
...
exports.syncTables = function(callback) {
	...
	var seriesCalls = [];
	for(modelName in Cassie.models) {
		seriesCalls.push(function(cb) {
			Sync.syncTable(Cassie.schemas[modelName], Cassie.models[modelName], (options.connection || Cassie.connection || null), Cassie.keyspace, Cassie.config, cb);
		});
	}
