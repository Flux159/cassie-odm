...
function Model(doc, docOptions, modelName, schema, connection, options) {
	...
	if(doc) {
		...
		if(typeof doc !== 'object') {
			throw "new Model(object) must have an object as its first argument.";
		}
		...
		if(options.pluralize) {
			...
			this.find = function(arguments, findOptions, callback) {
			...
			this.findById = function(id, options, callback) {
				...
				// if(!options) {
					// options = {limit: 1};
				// }
				this.find(args, options, callback);
			};
			this.findByIdAndUpdate = function(id, options, callback) {
				
			};
			this.findByIdAndRemove = function(id, options, callback) {
				
			};
			this.findOne = function(args, options, callback) {
			...
			this.findOneAndRemove = function() {
