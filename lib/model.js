...
function parseArguments(args, primaryKey) {
    ...
    for (var key in args) {
        ...
        if (typeof value === 'boolean') {
            argStringArray.push(' ' + key + ' =? ');
            argValueArray.push(value ? 'true' : 'false');
        }
        ...
        if (typeof value === 'object' && !(value instanceof Date) && !(value instanceof Long)) {
            ...
            if (Object.prototype.toString.call(value) === '[object Array]') {
                ...
                // Can also be $gte: String or Num and $lte: String or Num
...
var ModelSchema = function (modelName, schema, connection, options) {
    ...
    function Model(doc, docOptions) {
        ...
        for (var schemaField in schema._fields) {
            if (utils.isArray(this._schema._fields[schemaField])) {
                this[schemaField] = [];
                // reset dirty flag after setting field to the default empty array.
                this._is_dirty = {};
            } else if ((utils.isObject(this._schema._fields[schemaField])) && ((utils.isObject(this._schema._fields[schemaField].type)))) {
                this[schemaField] = {};
                // reset dirty flag after setting field to the default empty array.
                this._is_dirty = {};
            }
    ...
    Model.findById = function (id, findOptions, callback) {
        ...
        var args = { id: id };
        return Model.find(args, findOptions, callback);
    };
    Model.findOne = function (args, findOptions, callback) {
        ...
        if (findOptions) {
          var optionsType = typeof findOptions;

          if ((optionsType !== 'object') && (optionsType !== 'string')) {
            throw new Error('Options must be string containing fields to select or object. See documentation on options for Model.findOne() for more information.')
          }
        }

        var opts = (typeof findOptions === 'string')
          ? { fields: findOptions, limit: 1 }
          : Object.assign({ limit: 1 }, findOptions);

        if (!callback) {
            return Model.find(args, opts);
        } else {
            Model.find(args, opts, function (err, results) {
