...
// 				}) (doc);
				...
				if(doc) {
					...
					for(var field in doc) {
						...
						// console.log("Start setting this field: " + field);
						
						// this._fields[field] = doc[field];
						// fieldNames.push(field);
						
						// Model.define_fields(doc);
						
						this[field] = doc[field];
						// console.log("End Setting this field: " + field);
						
						// var stack = null;
						// this.__defineSetter__(field, function(newField) {
...
// 						});
					...
					// console.log("Should be done making new fields");
				...
				this.remove = function(removeOptions, callback) {
					...
					//TODO: Update this when I want to actually run a query
					// callback();
