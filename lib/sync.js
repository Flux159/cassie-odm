	...
	for(var tableName in createTablesDictionary) {
		...
		for(var tableKey in schema) {
			if(!(tableKey === '_sync' || tableKey === '_primary')) {
				...
				var keyType = determineKeyType(schema[tableKey]);
				...
				if(typeof schema[tableKey] === 'string') {
					primaryKeyString = "PRIMARY KEY (" + schema[tableKey] + ")";
				} else if(typeof schema[tableKey] === 'object') {
					primaryKeyString = primaryKeyString + "PRIMARY KEY (";
					//For each item inside the array, add to primary key string
					schema[tableKey].forEach(function(primary_key_item, index) {
						if(index === 0) {
...
exports.updateTables = function(updateTablesDictionary, connection, options, callback) {
	...
	// 			}
				...
				// var fieldsToAdd = schema;

				var fieldsToAdd = {};
				var undefinedFields = {};

				for(var field in schema) {
					fieldsToAdd[field] = schema[field];
	...
	//For each table, UPDATE it (need to describe the table, get columns, and only add columns that don't exist using ALTER command)
	
	//Also, write updated schema to file if options.schemaFile are passed - I don't know if this is absolutely necessary... I'm doing a check against the database for the columns, a file would just get outdated (and is a pain to handle...)
