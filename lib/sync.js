...
exports.checkKeyspaceExists = function (client, keyspace, options, execOptions, callback) {
    ...
    var query = "CREATE KEYSPACE IF NOT EXISTS " + keyspace + " WITH REPLICATION = " + replicationStrategy;
    ...
    if (execOptions.debug) {
        ...
        logger.info("CQL Create Keyspace Query: " + query);
        if (execOptions.prettyDebug) {
            logger.info("-----");
        }
    }
    ...
    Object.keys(createTablesDictionary).forEach(function (tableName) {
        ...
        seriesCalls.push(function (cb) {
            connection.execute(createTableQuery, [], function (err, results) {
                ...
                if (options.debug) {
                    if (err) {
                        logger.log("CQL Create Table Error: " + err);
                    }
                    logger.log("CQL Create Table Results: " + JSON.stringify(results));
                    if (options.prettyDebug) {
                        logger.log("-----");
                    }
                }
                ...
                connection.execute(describeTableQuery, describeTableArguments, function (err, results) {
                        ...
                        Object.keys(fieldsToAddIndex).forEach(function (indexField) {
                            ...
                            if (options.debug) {
                                ...
                                logger.log("CQL Create Index Query: " + createIndexString);
                                if (options.prettyDebug) {
                                    logger.log("-----");
                                }
                            }
                            ...
                            seriesCalls.push(function (cb) {
                                connection.execute(createIndexString, [], function (err, results) {
                                    cb(err, results);
                                });
                            });
